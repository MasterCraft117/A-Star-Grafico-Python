...
class Cell:
    ...
    def Display(self, screen, w, h, _s, showText=False):
        if self.itsObstacle:
            ...
            pygame.draw.rect(screen, startColor,
                             pygame.Rect(self.x * w, self.y * h, _s - cell_offset, _s - cell_offset))
        else:
...
def Drawline(cells, w, h, screen, color=(255, 255, 100)): #Dibuja la linea de ruta mas rapida, se cambio el color a amarillo, de un azul original 17,70,245 a amarillo 255,255,100
    for i in range(len(cells)):
        if i + 1 < len(cells):
            pygame.draw.line(screen, color, (int(cells[i].x * w) + w // 2, int(cells[i].y * h) + h // 2),
                             (int(cells[i + 1].x * w) + w // 2, int(cells[i + 1].y * h) + h // 2), 2)
